...
/// Answers a browser's hosting query with the native pairing state; the
/// Traverse host owns that state, so the listener is only handed a way to
/// reach it.
pub type HostingHandler =
    Arc<dyn Fn(tcode_protocol::HostingAction) -> tcode_protocol::HostingState + Send + Sync>;

/// The browser listener: static bundle, password login and `/ws`. It is
/// plain HTTP for direct access on this machine or its LAN; native devices
/// use Traverse.
pub struct RemoteConfig {
    ...
    /// Password login for the served browser.
    pub browser_password: bool,
    pub hosting: Option<HostingHandler>,
}

/// The device fields a browser sends with `/auth/login` and hello.
                    ...
                    match next {
                        Next::Accepted(Ok((stream, peer))) => {
                            ...
                            smol::spawn(async move {
                                ...
                                let result = admit(stream, shared).await;
...
async fn admit<S>(stream: S, shared: Arc<Shared>) -> io::Result<()>
where
...
async fn handle_connection<S>(stream: S, shared: Arc<Shared>) -> io::Result<()>
where
    S: futures_lite::io::AsyncRead + futures_lite::io::AsyncWrite + Unpin + Send,
{
    ...
    futures_lite::future::race(dispatch(stream, request, &shared), async {
        let _ = shared.shutdown.recv().await;
        Ok(())
    })
    .await
...
async fn dispatch<S>(mut stream: S, request: Request, shared: &Arc<Shared>) -> io::Result<()>
where
    S: futures_lite::io::AsyncRead + futures_lite::io::AsyncWrite + Unpin + Send,
{
    ...
    S: futures_lite::io::AsyncRead + futures_lite::io::AsyncWrite + Unpin + Send,
{
    ...
    let hello = futures_lite::future::race(receive_hello(&mut websocket), async {
        smol::Timer::after(Duration::from_secs(5)).await;
        None
    })
    .await;
    ...
    loop {
        ...
        match input {
            Input::WebSocket(Some(Ok(Message::Text(line)))) => {
                ...
                {
                    let result = match &shared.hosting {
                        Some(hosting) => {
                            Ok(tcode_protocol::QueryResponse::Hosting(hosting(action)))
                        }
                        None => Err(tcode_protocol::ProtocolError {
                ...
                if let Some(key) = value.get("key").and_then(serde_json::Value::as_str) {
                    ...
                    let device = crate::auth::hex_hash(token.as_bytes());
...
async fn receive_hello<S>(websocket: &mut WebSocketStream<S>) -> Option<Hello>
where
