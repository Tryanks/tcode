// Mobile port: platform fallbacks, Android aliases, and host color-emoji rasterization.
...
/// A platform-rendered colour glyph in RGBA byte order.
...
type ColorEmojiRasterizer = Arc<dyn Fn(u32, f32) -> Option<ColorGlyphRaster> + Send + Sync>;
...
fn merged_font_fallbacks(
    requested: Option<&FontFallbacks>,
    platform_defaults: &[String],
) -> Option<FontFallbacks> {
    let mut families = requested
    ...
    (!families.is_empty()).then(|| FontFallbacks::from_fonts(families))
}
...
struct CosmicTextSystemState {
    ...
    default_font_fallbacks: Vec<String>,
    color_emoji_rasterizer: Option<ColorEmojiRasterizer>,
}
...
impl CosmicTextSystem {
    pub fn new(system_font_fallback: &str) -> Self {
        ...
        Self(RwLock::new(CosmicTextSystemState {
            ...
            default_font_fallbacks: Vec::new(),
            color_emoji_rasterizer: None,
        }))
    }
    ...
    /// Appends platform fallback families to every requested font.
    pub fn set_default_font_fallbacks(&self, fallbacks: Vec<String>) {
        let mut state = self.0.write();
        state.default_font_fallbacks = fallbacks;
        state.font_ids_by_family_cache.clear();
    }

    /// Installs a last-resort colour emoji rasterizer for unsupported formats.
    pub fn set_color_emoji_rasterizer(
        &self,
        rasterizer: impl Fn(u32, f32) -> Option<ColorGlyphRaster> + Send + Sync + 'static,
    ) {
        self.0.write().color_emoji_rasterizer = Some(Arc::new(rasterizer));
    }
}
...
impl PlatformTextSystem for CosmicTextSystem {
    ...
    fn font_id(&self, font: &Font) -> Result<FontId> {
        let mut state = self.0.write();
        let fallbacks = merged_font_fallbacks(
            font.fallbacks.as_ref(),
            state.default_font_fallbacks.as_slice(),
        );
        let key = FontKey::new(
            font.family.clone(),
            font.features.clone(),
        ...
        let candidates = if let Some(font_ids) = state.font_ids_by_family_cache.get(&key) {
            ...
            let font_ids = state.load_family(&font.family, &font.features, fallbacks.as_ref())?;
...
impl CosmicTextSystemState {
    ...
    fn load_family(
        ...
        let name = match name {
            // Android's CSS-style aliases are not literal OpenType family names.
            "monospace" | "ui-monospace" => "Droid Sans Mono",
            "sans-serif" | "system-ui" => &self.system_font_fallback,
            _ => name,
        };
    ...
    fn render_glyph_image(
        ...
        let swash_image = renderer.render(&mut scaler, glyph_id);
        let swash_image_is_empty = swash_image.as_ref().is_none_or(|image| {
            image.placement.width == 0 || image.placement.height == 0 || image.data.is_empty()
        });

        // Empty rasters are valid for spacing glyphs. For emoji they can mean
        // that Swash cannot decode the platform font's colour format.
        if !params.is_emoji || !swash_image_is_empty {
            return swash_image
                .with_context(|| format!("unable to render glyph via swash for {params:?}"));
        }

        if let Some(rasterizer) = &self.color_emoji_rasterizer
            && let Some(raster) = rasterizer(params.glyph_id.0, pixel_size * params.scale_factor)
        {
            return Ok(swash::scale::image::Image {
