// Mobile port: expose the scale-factor hook required by iOS and Android windows.
...
impl WgpuRenderer {
    ...
    /// Updates the display scale. Scene geometry is already in device pixels,
    /// so the upstream non-glass renderer has no scale-dependent state.
