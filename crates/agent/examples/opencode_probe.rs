//! Native OpenCode server/REST/SSE smoke probe.
//!
//! ```text
//! cargo run -p agent --example opencode_probe -- "Reply with exactly PONG" /tmp /opt/homebrew/bin/opencode
//! ```
...
async fn run(prompt: String, cwd: PathBuf, binary_path: Option<PathBuf>) -> i32 {
    ...
    .await
    {
        Ok(models) => {
            ...
            eprintln!(
                "opencode_probe: configured model count={} selected={default:?}",
                models.len()
            );
            default
        }
        Err(error) => {
            eprintln!("opencode_probe: model discovery failed: {error}");
            None
        }
    };
    ...
    .await
    {
        Ok(handle) => handle,
        Err(error) => {
            eprintln!("opencode_probe: startup failed: {error}");
            return 1;
        }
    };
    if handle
        ...
        .is_err()
    {
        eprintln!("opencode_probe: command channel closed before prompt");
        return 1;
    }
    ...
    loop {
        ...
        let Some(event) = event else {
            eprintln!("opencode_probe: timed out");
            break;
        };
        println!(
            "{}",
