...
mod tests {
    ...
    /// `tcode_services::process::command` / `async_command` instead of
    ...
    fn no_direct_command_new_outside_this_module() {
        ...
        let crates_dir = manifest_dir
            .parent()
            .expect("services manifest directory must be nested under workspace/crates")
            .to_path_buf();
        assert!(
            crates_dir.is_dir(),
        ...
        let exempt = [
            crates_dir.join("services/src/process.rs"),
            crates_dir.join("agent/src/process.rs"),
        ];
        for root in &roots {
            assert!(
                root.is_dir(),
        ...
        assert!(
            offenders.is_empty(),
            "spawn sites bypassing tcode_services::process (Windows would pop a console): {offenders:#?}"
        );
    }

    fn visit(dir: &std::path::Path, exempt: &[std::path::PathBuf], offenders: &mut Vec<String>) {
