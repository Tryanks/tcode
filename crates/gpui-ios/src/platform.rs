...
use crate::{
    IosDisplayMetrics, IosHost, IosSurface,
    dispatcher::IosDispatcher,
    display::IosDisplay,
    keyboard::IosKeyboardLayout,
    window::{IosWindow, IosWindowInner},
};
...
struct IosPlatformState {
    ...
    window: Option<Weak<IosWindowInner>>,
    appearance: WindowAppearance,
    thermal_state: ThermalState,
    callbacks: IosPlatformCallbacks,
}
...
pub struct IosPlatform {
    ...
    state: Rc<RefCell<IosPlatformState>>,
    gpu_context: GpuContext,
}

impl IosPlatform {
    /// Creates the backend on Android's UI thread with the current surface generation.
    pub fn new(host: Arc<dyn IosHost>, surface: IosSurface) -> Result<Self> {
        ...
        let display = Rc::new(IosDisplay::new(surface.metrics));
        ...
        Ok(Self {
            ...
            text_system,
            display,
            state: Rc::new(RefCell::new(IosPlatformState {
                surface: Some(surface),
                active_window: None,
                window: None,
                appearance,
                thermal_state,
                callbacks: IosPlatformCallbacks::default(),
            })),
...
impl IosEventSink {
    /// Drains foreground tasks after `IosHost::wake_main_thread`.
    ...
    pub fn surface_created(&self, surface: IosSurface) -> Result<()> {
        ...
        if let Some(window) = self.window() {
            window.replace_surface(surface)?;
        }
    ...
    fn window(&self) -> Option<Rc<IosWindowInner>> {
...
impl Platform for IosPlatform {
    ...
    fn run(&self, on_finish_launching: Box<dyn 'static + FnOnce()>) {
        assert!(
            self.dispatcher.is_main_thread(),
            "IosPlatform::run must be called on the activity UI thread"
        );
    ...
    fn open_window(
        ...
        if !matches!(&params.kind, WindowKind::Normal) {
            bail!("iOS supports only its single scene window");
        }
        if self
            .state
            .borrow()
        ...
        // Copy, not clone: IosSurface is a view pointer plus metrics, unlike
        // Android's ref-counted ANativeWindow wrapper.
        let surface = self
            .state
            .borrow()
            .surface
            .context("no CAMetalLayer is currently available")?;
    ...
    fn open_with_system(&self, path: &Path) {
        if let Err(error) = self.host.open_url(&format!("file://{}", path.display())) {
