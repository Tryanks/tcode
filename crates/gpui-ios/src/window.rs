...
use crate::{IosDisplayMetrics, IosHost, IosLayer, IosSurface, display::IosDisplay};
...
struct IosWindowState {
    ...
    native_window: Option<IosLayer>,
    retired_native_window: Option<IosLayer>,
...
pub(crate) struct IosWindow {
    pub(crate) inner: Rc<IosWindowInner>,
...
impl IosWindow {
    pub(crate) fn new(
        ...
        .context("failed to create the iOS Metal renderer")?;
...
impl IosWindowInner {
    pub(crate) fn replace_surface(&self, surface: IosSurface) -> Result<()> {
        let instance = self
            .gpu_context
            .borrow()
            .as_ref()
            .context("wgpu context missing while replacing iOS drawable")?
            .instance
            .clone();
    ...
    pub(crate) fn resize_from_native(&self, metrics: IosDisplayMetrics) {
        {
            let mut state = self.state.borrow_mut();
    ...
    fn notify_resize(&self, metrics: IosDisplayMetrics) {
...
impl HasWindowHandle for IosWindow {
    fn window_handle(&self) -> Result<WindowHandle<'_>, HandleError> {
        ...
        // SAFETY: IosWindowInner retains the corresponding NativeWindow
...
impl PlatformWindow for IosWindow {
    ...
    fn resize(&mut self, _size: Size<Pixels>) {
        // UIKit owns the drawable's dimensions; layout callbacks update GPUI.
    }
    ...
    fn prompt(
        ...
        // UIAlertController is presented asynchronously, which GPUI's synchronous prompt API cannot express.
        None
    }

    fn activate(&self) {
        // iOS, not an application window, owns foreground activation.
    }
    ...
    fn set_title(&mut self, title: &str) {
        // iOS takes its task label from Info.plist; the title is retained for GPUI's own queries.
    ...
    fn minimize(&self) {
        // iOS scene visibility is lifecycle-owned; there is no minimize.
    }

    fn zoom(&self) {
        // iOS has no desktop maximize or zoom operation.
    }
    ...
    fn completed_frame(&self) {
        // WgpuRenderer presents the iOS drawable as part of draw.
    }
    ...
    fn is_subpixel_rendering_supported(&self) -> bool {
        // iOS display rotation makes RGB stripe order unstable; use grayscale glyphs.
        false
    }
...
fn surface_config(metrics: IosDisplayMetrics, transparent: bool) -> WgpuSurfaceConfig {
    WgpuSurfaceConfig {
        size: Size {
            width: DevicePixels(metrics.width_px as i32),
            height: DevicePixels(metrics.height_px as i32),
