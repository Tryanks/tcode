...
mod tests {
    ...
    struct LayeredSelectionRoot {
        sidebar: Entity<crate::sidebar::SessionsSidebar>,
        markdown: Entity<MarkdownState>,
    }

    impl Render for LayeredSelectionRoot {
            ...
            div()
                ...
                .child(
                    div()
                        .flex()
                        .size_full()
                        .child(div().w(px(300.)).flex_none().child(self.sidebar.clone()))
                        .child(
                            div()
                                .flex_1()
                                .p_2()
                ...
                .child(
                    div()
                        ...
                        .right_0()
                        .top(px(400.))
                        .debug_selector(|| "selection-popover-trigger".into())
                        .child(
                            crate::widgets::Popover::new("selection-probe-popover")
                                .trigger(
                                    crate::widgets::Button::new("selection-popover-button")
                                        .label("Popover"),
                                )
                                .content(|_, _, _| {
                                    div()
                                        .w(px(120.))
                                        .debug_selector(|| "selection-popover-action".into())
                                        .child(
                                            crate::widgets::Button::new("selection-popover-action")
                                                .label("Keep open"),
                                        )
                                }),
                        ),
    ...
    #[gpui::test]
    fn sidebar_and_overlay_gestures_do_not_start_chat_selection(cx: &mut TestAppContext) {
        use crate::overlay::{Notification, OverlayExt as _, OverlayHost};
        use tcode_runtime::pipe::{HostServices, spawn_host};
        use tcode_services::store::SessionStore;
        cx.update(crate::theme::init);
        cx.update(crate::markdown::init);
        let profile = std::env::temp_dir().join(format!(
            "tcode-selection-layers-{}",
            tcode_services::store::now_millis()
        ));
        let host = spawn_host(
            SessionStore::open_at(profile.clone()).unwrap(),
            HostServices::default(),
        )
        .unwrap();
        let mut project = tcode_core::project::Project::from_root(profile.join("project"));
        project.id = "selection-fixture".into();
        smol::block_on(host.update_state_for_test(move |state, _| {
            state.settings.sidebar_layout = tcode_core::settings::SidebarLayout::Grouped;
        ...
        let (_, cx) = cx.add_window_view(|window, cx| {
            let body = cx.new(|cx| {
                let store = cx.new(|cx| crate::store::WorkspaceStore::new(host.link(), cx));
                let window_state = cx.new(|_| crate::window_state::WindowState::new(false));
                LayeredSelectionRoot {
                    sidebar: cx
                        .new(|cx| crate::sidebar::SessionsSidebar::new(store, window_state, cx)),
                    markdown: cx.new(|cx| {
                        MarkdownState::new(&"Alpha beta gamma delta epsilon\n\n".repeat(15), cx)
                    }),
                }
            });
        ...
        draw(cx);
        let header = cx.debug_bounds("project-header-selection-fixture").unwrap();
        cx.simulate_mouse_down(header.center(), MouseButton::Left, Modifiers::default());
        ...
        assert!(
            cx.update(gpui_base::TextSelection::selected_text)
                .is_empty(),
            "dragging from the sidebar into chat cannot start text selection"
        );
        ...
        for surface in ["sidebar", "dialog", "popover", "notification"] {
            cx.update(gpui_base::TextSelection::clear);
            let target = match surface {
                "sidebar" => cx
                    .debug_bounds("project-header-selection-fixture")
                    .unwrap()
                    .center(),
                "dialog" => {
                    cx.update(|window, cx| {
                        window.open_dialog(cx, |dialog, _, _| {
                            dialog.title("Selection probe").child(
                                div()
                                    .debug_selector(|| "selection-dialog-action".into())
                                    .child(
                                        crate::widgets::Button::new("probe-dialog")
                                            .label("Keep open"),
                                    ),
                            )
                        })
                    ...
                    draw(cx);
                    cx.debug_bounds("selection-dialog-action").unwrap().center()
                }
                "popover" => {
                    let trigger = cx.debug_bounds("selection-popover-trigger").unwrap();
                    cx.simulate_click(trigger.center(), Modifiers::default());
                    draw(cx);
                    cx.debug_bounds("selection-popover-action")
                        .unwrap()
                        .center()
                }
                _ => {
                    cx.update(|window, cx| {
                        window.push_notification(
                            Notification::info("Selection probe").autohide(false),
                            cx,
                        )
                    });
                    draw(cx);
                    ...
                    draw(cx);
                    let toast = cx.debug_bounds("wide-toast").unwrap();
                    point(toast.right() - px(14.), toast.top() + px(14.))
                }
            };
            ...
            if surface == "dialog" {
                ...
                draw(cx);
                assert!(
                    cx.debug_bounds("wide-toast").is_none(),
                    "the actual notification close button was clicked"
                );
            }
            cx.simulate_mouse_move(point(px(420.), px(120.)), None, Modifiers::default());
            draw(cx);
            let selection = cx.update(gpui_base::TextSelection::selected_text);
            assert!(
                selection.is_empty(),
                "{surface} gesture armed chat selection: {selection:?}"
            );
        }
        cx.update(gpui_base::TextSelection::clear);
        ...
        draw(cx);
        let dragged = cx.update(gpui_base::TextSelection::selected_text);
        assert!(!dragged.is_empty(), "legitimate markdown drag selects text");
        cx.update(|window, cx| {
            window.open_dialog(cx, |dialog, _, _| {
                dialog.title("Release over dialog").child("Dialog content")
            })
        });
        draw(cx);
        cx.simulate_mouse_up(
        ...
        cx.update(|window, cx| window.close_dialog(cx));
        draw(cx);
        let released = cx.update(gpui_base::TextSelection::selected_text);
        cx.simulate_mouse_move(point(px(450.), px(240.)), None, Modifiers::default());
        draw(cx);
        assert_eq!(
            cx.update(gpui_base::TextSelection::selected_text),
            released,
            "release owned by a dialog must terminate selection before button-up movement"
        );
        cx.update(gpui_base::TextSelection::clear);
        cx.simulate_mouse_down(
            point(px(310.), px(15.)),
        ...
        draw(cx);
        let before_deactivation = cx.update(gpui_base::TextSelection::selected_text);
        ...
        assert_eq!(
            cx.update(gpui_base::TextSelection::selected_text),
            before_deactivation,
            "focus loss cancels the selection gesture"
        );
