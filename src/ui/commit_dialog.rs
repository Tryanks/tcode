...
use crate::git::{GitAction, GitFileEntry};
...
impl CommitDialog {
    pub fn new(
        ...
        let message = cx.new(|cx| {
            InputState::new(window, cx)
                .multi_line(true)
                .auto_grow(3, 10)
                .placeholder(rust_i18n::t!("git.commit.message_placeholder"))
        });
        let mut this = Self {
            app_state,
            message,
    ...
    fn included(&self) -> Option<Vec<String>> {
        crate::git::included_paths(&self.files, &self.excluded)
    }
    ...
    pub fn confirm(&mut self, window: &mut Window, cx: &mut Context<Self>) -> bool {
        ...
        let feature_branch = if self.on_default_branch && self.create_feature_branch {
            Some(crate::git::feature_branch_name(
    ...
    pub fn confirm_label(&self, cx: &App) -> String {
        ...
        match self.action {
            GitAction::CommitPush => rust_i18n::t!("git.commit.confirm_push").into_owned(),
            _ => rust_i18n::t!("git.commit.confirm").into_owned(),
        }
    }
...
impl Render for CommitDialog {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let branch_label = self
            .branch
            .clone()
            .unwrap_or_else(|| rust_i18n::t!("git.commit.detached").into_owned());
                ...
                v_flex()
                    ...
                    .child(
                        h_flex()
                            ...
                            .child(rust_i18n::t!("git.commit.default_warning_title")),
                    )
                    .child(
                        div()
                            .text_size(px(12.))
                            .text_color(muted)
                            .child(rust_i18n::t!("git.commit.default_warning_body")),
                    )
                    .child(
                        Checkbox::new("commit-feature-branch")
                            .checked(create)
                            .label(rust_i18n::t!("git.commit.create_feature_branch").into_owned())
        ...
        if self.files.is_empty() {
            file_rows = file_rows.child(
                div()
                    ...
                    .text_size(px(12.))
                    .text_color(muted)
                    .child(rust_i18n::t!("git.commit.no_changes")),
            );
        } else {
        ...
        let message_header = h_flex()
            ...
            .child(
                div()
                    ...
                    .font_medium()
                    .text_color(muted)
                    .child(rust_i18n::t!("git.commit.message_label")),
            )
            .child(
                Button::new("commit-regenerate")
                    ...
                    .label(if self.generating {
                        rust_i18n::t!("git.commit.generating")
                    } else {
                        rust_i18n::t!("git.commit.regenerate")
                    })
