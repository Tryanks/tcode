...
- **排队消息重绘为幽灵气泡**：废除现在 composer 上方的带边框卡片盒（盒中盒）。
  每条排队消息渲染为右对齐的用户气泡同形态、整体 opacity ≈0.55（BUI 未定格内容
  的标准处理），下缘一行 11px mono 元数据（定时消息显示倒计时）；悬停浮现操作
  （立即发送/引导、删除 ×，复用现有按钮逻辑与 tooltip）。"N 条排队中"标题行仅在
  >2 条时出现，10.5px 大写小节样式。不加容器边框，气泡自身即容器。
